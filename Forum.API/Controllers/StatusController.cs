...
namespace ReportService.API.Controllers
{
    ...
    public class StatusController : ControllerBase
    {
        ...
        //GET: api/events
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Status>>> GetAllStatusAsync()
        {
            try
            {
                var results = await _statusService.GetAllStatusesAsync();
                _logger.LogInformation($"Данні успішно отримано");
                return Ok(results);
            }
            catch (Exception ex)
            {
                _logger.LogError($"Транзакція поломалась! в цьому винен метод GetAllStatusAsync() - {ex.Message}");
        ...
        //POST: api/events
        [HttpPost]
        public async Task<ActionResult> PostEventAsync([FromBody] Status status)
        {
            try
            {
                if (status == null)
                ...
                var created_id = await _statusService.AddStatusAsync(status);
        ...
        //POST: api/events/id
        [HttpPut("{id}")]
        public async Task<ActionResult> UpdateEventAsync(int id, [FromBody] Status evnt)
        {
            try
            {
                if (evnt == null)
                ...
                var event_entity = await _statusService.GetStatusByIdAsync(id);
                ...
                await _statusService.UpdateStatusAsync(id, evnt);
